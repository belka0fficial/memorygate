...
`pending_clarifications` all carry an `agent_id` column, and every query
...
| Table | Purpose |
|---|---|
| `memories` | Durable facts/preferences. Has `agent_id`, `memory_type`, `source_type`, `confidence`, `identity_weight`, `tags_json`, `created_at`/`updated_at`. |
...
| `observations` | Single signal instances. Has `agent_id`, `signal_type` (verbal / tonal / behavioral / physical / timing / anything else the caller passes), `status` (unconfirmed / confirmed / contradicted / archived), `hypothesis` + `hypothesis_confidence`, `confirmation_count`, `exposure_count`/`max_exposures`, `trigger_context`, `archived_at`/`archive_reason`. |
...
| `pending_clarifications` | Ambiguous signals queued to ask about later instead of guessing. Has `agent_id`, `importance` (drives briefing ordering), `status` is pending / asked / resolved / dismissed, with `ask_after` gating when it's safe to bring up. |
...
after `create_all` and additively patches pre-agent-isolation databases:
adds the `agent_id` columns, renames `entities.conker_notes`/`conker_summary`
to `agent_notes`/`agent_summary`, and adds the observation-lifecycle and
clarification-importance columns. All of it is `IF NOT EXISTS`-guarded, so
it's a no-op on a fresh database (which already gets the new schema
straight from the model definitions) and safe to run repeatedly.
...
`services/classifier.py` is a **scored multi-signal keyword classifier**,
still not an LLM call. Each candidate `memory_type` is defined by one or
more independent keyword-list "signals"; a signal fires (weight 1.0) if any
phrase in its list is present (some are gated by an exclusion list instead,
e.g. identity traits are disqualified by temporary-language phrasing). The
type with the most fired signals wins, ties broken by priority order
(`harmful_pattern` > `stable_preference` > `identity_trait` > `humor_style`
> `temporary_phase` > `support_context`); confidence scales with signal
count (1 → low, 2 → medium, 3+ → high), and `identity_weight` bumps from
medium to high when confidence is high. No match at all falls back to
`task_context` (medium confidence, low identity_weight).

`memory_type`s in play now: `stable_preference`, `identity_trait`,
`humor_style`, `temporary_phase`, `harmful_pattern`, `support_context`,
`task_context`. `harmful_pattern` also fires on `source_type ==
"soulgate_inferred"` as one of its two signals.
...
event "streaks", top-2 pending clarifications by `importance`, in-progress
`project` entities touched in the last 21 days, `human` entities with
recent activity or `attributes.scheduled_for == today`, and recent
`harmful_pattern` memories as watch flags. Several fields ride on
...
### `/memory`
- `POST /memory/write` — `{agent_id?, text, source_type?, memory_type?, confidence?, identity_weight?, tags?}` → signal filter, then classifies (or accepts overrides), dedupes (exact text, then agent-scoped vector novelty check), upgrades or inserts, returns `{status, id, memory_type, summary, upgraded?, duplicate_of?, near_duplicate_of?, similarity?, low_novelty?}` or `{status: "filtered", reason: "low value"}`.
...
- `PATCH /memory/{id}` — `{text?, memory_type?, confidence?, identity_weight?, tags?}` partial edit; re-embeds and logs an `edit` audit row.
...
### `/observation`
- `POST /observation/create` — `{agent_id?, session_id?, signal_type, description, raw_context?, hypothesis?, hypothesis_confidence?, status?, confirmed_by?, trigger_context?, max_exposures?, entity_ids?, related_observation_ids?}` — dedup-checks first (see "Observation lifecycle" above); on a miss, enforces the active-observation budget, then inserts and runs pattern promotion.
- `POST /observation/search` — `{agent_id?, query?, signal_type?, status?, entity_id?}`, agent-scoped at the SQL level, then filtered/substring-matched in Python, capped at 50 results.
- `GET /observation/active` — all non-archived observations for the caller's agent.
...
- `POST /observation/update/{id}` — partial update (status, hypothesis, hypothesis_confidence, confirmed_by, trigger_context, related_observation_ids).
...
- `POST /clarification/create` — now also takes `importance` (0–1, default 0.5), used to rank clarifications in the briefing.
- `POST /clarification/search` — query is tokenized and every token must appear somewhere in the haystack (AND match, not substring), agent-scoped.
...
- **Overview** — four stat cards (memories/entities/active patterns/pending clarifications), three signal-health meters (observation→pattern promotion rate, signal-filter rejection rate, memory novelty rate — all derived client-side from `/audit` + list endpoints, see the `/audit` note above), and a merged activity feed (last 20 events across memories/entities/observations/patterns/clarifications, newest first).
- **Memories** — debounced (300ms) semantic search with similarity % on results, type/confidence/sort filters, 2-column cards, a right-side detail panel (edit text/type/confidence/identity_weight/tags, delete), and an Add Memory modal.
...
- **Observations** — Active/Confirmed/Contradicted/Archived tabs (the four `status` values, "Active" being the friendlier label for `unconfirmed`), signal-type filter, an exposure bar color-coded by how close `exposure_count` is to `max_exposures`, an amber "Pattern candidate" flag at `confirmation_count >= 3`, click-to-expand-inline (not a side panel) for the full edit form + delete, and an Add Observation modal with debounced entity search for `entity_ids`.
- **Patterns** — Active and collapsible Candidates sections, confirmation/contradiction count bars, a derived confidence % (`confirmations / (confirmations + contradictions)`, falling back to the stored `confidence` when both are zero), "N more confirmations to activate" on candidates, Confirm/Contradict/Dismiss (Dismiss sets `status="deprecated"` via `POST /pattern/update/{id}` — there's no dedicated dismiss endpoint, candidates just don't have one, unlike observations).
- **Clarifications** — Pending and Resolved tabs (Resolved covers both `resolved` and `dismissed`, since there's no third tab for the latter), pending cards show hypotheses as a bullet list and `ask_after` as a muted "raise if …" line, Mark Resolved opens a modal asking for the actual answer, Archive sets `status="dismissed"` directly from the card.
...
Mobile: bottom tab bar, capped at 5 visible tabs (Overview/Memories/
Entities/Observations + a "More" tab that expands Patterns/Clarifications/
Briefing) rather than cramming all 7 in.
...
- `screenshot-interact.mjs <url> [label] [agent] [key] [actions] [w] [h]`
  ...
  (for SVG/canvas targets `clickText` can't reach, e.g. graph nodes),
  `wait:<ms>`.
...
- `classifier.py` and `signal_filter.py` are pure keyword/heuristic
  ...
  falls back to `task_context`; `pattern_promotion.py`'s "similar
  hypothesis" clustering is exact-normalized-text matching, not real
  semantic similarity.
...
- `/observation/search`, `/pattern/search`, and `/clarification/search`
  all load their full table (now agent-scoped in SQL first, at least) and
  filter/substring-match the rest in Python — fine at current scale, won't
  scale past a few thousand rows per agent.
